...
import (
	...
	"k8s.io/autoscaler/cluster-autoscaler/utils/readiness"
...
func (a *StaticAutoscaler) RunOnce(currentTime time.Time) errors.AutoscalerError {
	...
	for _, po := range unschedulablePods {
		...
		for _, vol := range po.Spec.Volumes {
			...
			pvc, err := a.PredicateChecker.PersistentVolumeClaimLister.PersistentVolumeClaims(po.Namespace).Get(vol.PersistentVolumeClaim.ClaimName)
			if err != nil {
				volumes = append(volumes, vol)
				continue
			}
...
func (a *StaticAutoscaler) obtainNodeLists(cp cloudprovider.CloudProvider) ([]*apiv1.Node, []*apiv1.Node, errors.AutoscalerError) {
	...
	allNodes, readyNodes = readiness.FilterOutNodesWithLabels(allNodes, readyNodes)
